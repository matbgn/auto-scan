...
def auto_scan():
    ...
    if request.form['subject'] != '':
        ...
        _is_local_scan = request.form.get('is_local_scan', False)
        logger.warn("Param is_local: %s", _is_local_scan)

        main(';'.join(multiselect), request.form['subject'], request.form['scan_mode'],
             request.form['paper_format'], int(request.form['batch_total']), as_web_interface=True, is_local_scan=_is_local_scan)
